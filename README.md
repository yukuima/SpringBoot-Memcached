...
public class MemcachedApplicationTests {
    ...
    public void testReplace() {
        Boolean flag = memcachedManager.replace("someKey", "myk", 10000);
        if (flag) {
    ...
    @Test
    public void testAdd() {
        Boolean flag = memcachedManager.add("someKey", "myk", 10000);
        if (flag) {
            System.out.println("最终结果为:" + memcachedManager.get("someKey").toString());
            return;
        }
    ...
    public void asyncGetMultiTest() {
        memcachedManager.set("aa", 100000, "大牛");
        memcachedManager.set("bb", 100000, "大呆");
